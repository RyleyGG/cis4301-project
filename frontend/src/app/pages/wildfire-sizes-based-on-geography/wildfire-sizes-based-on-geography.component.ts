...
@Component({
  ...
  standalone: true,
  imports: [CalendarModule, FormsModule, ChartModule, InputTextModule, DropdownModule, MultiSelectModule],
  templateUrl: './wildfire-sizes-based-on-geography.component.html',
  styleUrl: './wildfire-sizes-based-on-geography.component.css'
})

export class WildfireSizesBasedOnGeographyComponent implements OnInit {
  ...
  submitTypesForm() {
    ...
    this.trendQueryService.getWildFireTypesBasedOnGeo(formData)
      ...
      .subscribe((res) => {
        ...
        res.forEach((obj) => {
          const key = `${obj.geographic_area_code ?? 'Unknown'}`;
